...
from toolkit import subscription
from toolkit import test_pubsub_util as tpsu
...
class SubscriptionTest(parameterized.TestCase):
  ...
  def setUp(self):
    ...
    self._subscriber = mock.MagicMock()
    self._sub_client = subscription.SubscriptionClient(
        _PROJECT_ID,
        _SUBSCRIPTION_ID,
        credentials=mock.MagicMock(),
        # Mock GRPC channel since it causes a memory leak because it is never
        # explicitly closed b/145752105.
        channel=mock.MagicMock())
    self._sub_client._subscriber = self._subscriber
