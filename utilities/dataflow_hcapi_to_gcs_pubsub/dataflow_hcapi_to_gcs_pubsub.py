...
class consumeTransformMessages(beam.DoFn):
    ...
    def process(self,element):
        ...
        try:
            ...
            logging.error("process to transform pub/sub record from HCAPI failed due to : {}".format(error, exc_info=True))
            errorMessage = "process to transform pub/sub record from HCAPI failed due to : {}".format(str(type(error).__name__)+" --> "+ str(error))
            rawMessage = dict()
