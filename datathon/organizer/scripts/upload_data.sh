...
# Lowercase is required for bucket name and deployment job name.
...
- name: big-query-dataset
  ...
  properties:
    ...
    location: "${LOCATION?}"
EOF
gcloud deployment-manager deployments create create-${DATASET_ID} \
  --config=${TEMP} --project ${PROJECT_ID}
# Delete the deployment job since it's finished. The created dataset is not
# affected.
gcloud --quiet deployment-manager deployments delete create-${DATASET_ID} \
