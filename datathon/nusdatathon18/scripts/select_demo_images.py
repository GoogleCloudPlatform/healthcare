...
r"""
Select train/test images for demo.
    ...
    for blob in blobs:
      if blob.name.endswith('_CC'):
        src_bucket.copy_blob(
          blob, dst_bucket,
          new_name=("%s/%s" % (args.dst_training_folder,
            blob.name.split('/')[1])))
...
def select_test_data(src_bucket, dst_bucket, args):
  ...
  for i in range(args.eval_size):
    src_bucket.copy_blob(
      objects[i], dst_bucket,
      new_name=("%s/%s" % (args.dst_eval_folder,
        objects[i].name.split('/')[1])))
...
if __name__ == '__main__':
  ...
  parser.add_argument('--src_training_folder', type=str, required=True,
    help='GCS folder to read training images from.')
  parser.add_argument('--src_eval_folder', type=str, required=True,
  ...
  parser.add_argument('--dst_training_folder', type=str, required=True,
    help='GCS folder to write demo training images to.')
  parser.add_argument('--dst_eval_folder', type=str, required=True,
    help='GCS folder to write demo evaluation images to.')
  parser.add_argument('--training_size_per_cat', type=int, required=True,
