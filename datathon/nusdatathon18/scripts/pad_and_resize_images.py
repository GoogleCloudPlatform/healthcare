...
r"""
Pads the images to the desired ratio with black pixels, and resize
...
def pad_and_resize_image(src_bucket, dst_bucket, blob, args):
  ...
  (Image.fromarray(mask.astype('uint32'))
    .resize((args.final_width, args.final_height))
    .save(upload_byte_stream, format='PNG'))
...
if __name__ == '__main__':
  ...
  parser.add_argument('--final_width', type=int, required=True,
    help='Final width to resize the images to.')
  parser.add_argument('--final_height', type=int, required=True,
