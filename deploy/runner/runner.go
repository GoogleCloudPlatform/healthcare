...
// DefaultRunner is the Runner that executes the command by default.
type DefaultRunner struct{}

// CmdRun executes the command.
func (r *DefaultRunner) CmdRun(cmd *exec.Cmd) error {
...
func (r *DefaultRunner) CmdOutput(cmd *exec.Cmd) ([]byte, error) {
	log.Printf("Running: %v", cmd.Args)
	var stderr bytes.Buffer
	cmd.Stderr = &stderr
	b, err := cmd.Output()
...
func (r *DefaultRunner) CmdCombinedOutput(cmd *exec.Cmd) ([]byte, error) {
	log.Printf("Running: %v", cmd.Args)
...
// FakeRunner is the Runner that only prints command information and does not run it.
type FakeRunner struct{}

// CmdRun prints the command information.
func (r *FakeRunner) CmdRun(cmd *exec.Cmd) error {
	log.Printf("Dry run call: %s", strings.Join(cmd.Args, " "))
	return nil
}

// CmdOutput prints the command information.
func (r *FakeRunner) CmdOutput(cmd *exec.Cmd) ([]byte, error) {
	log.Printf("Dry run call: %s", strings.Join(cmd.Args, " "))
...
func (r *FakeRunner) CmdCombinedOutput(cmd *exec.Cmd) ([]byte, error) {
	log.Printf("Dry run call: %s", strings.Join(cmd.Args, " "))
	switch cmdStr := strings.Join(cmd.Args, " "); {
