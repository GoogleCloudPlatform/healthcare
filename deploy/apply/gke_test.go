...
func TestGetGCloudCredentials(t *testing.T) {
	...
	if len(r.called) != 1 {
		t.Fatalf("getGCloudCredentials does not run correct number of commands: got %d, want 1", len(r.called))
	}
...
func TestApplyClusterResource(t *testing.T) {
	...
	if len(r.called) != 1 {
		t.Fatalf("applyClusterWorkload does not run correct number of commands: got %d, want 1", len(r.called))
	}
	if diff := cmp.Diff(r.called[0], wantCmd); len(diff) != 0 {
...
resources:
  ...
  - cluster_name: cluster1
	...
	r := &testRunner{}
	err := deployGKEWorkloads(project, r)
	if err != nil {
  ...
  properties: {}`}
	...
	for _, s := range wantArgSegs {
		if !strings.Contains(r.called[0], s) {
			t.Fatalf("binauthz commands does not contain wanted args: got %q, want %q", r.called[0], s)
		}
	}
