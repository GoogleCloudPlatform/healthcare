...
func (r *tfTestRunner) CmdRun(cmd *exec.Cmd) error { return nil }

func (r *tfTestRunner) CmdOutput(cmd *exec.Cmd) ([]byte, error) {
	const logSinkJSON = `{
...
func (r *tfTestRunner) CmdCombinedOutput(cmd *exec.Cmd) ([]byte, error) { return nil, nil }
...
- google_service_account:
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			if err := resources(project, &Options{ImportExisting: true, Runner: &tfTestRunner{}}); err != nil {
...
`),
	...
	if err := auditResources(config, project, &Options{ImportExisting: true, Runner: &tfTestRunner{}}); err != nil {
