...
func applyConfigs() (err error) {
	...
	if *dryRun {
		opts.Runner = &runner.FakeRunner{}
	} else {
		opts.Runner = &runner.DefaultRunner{}
	}
	...
	if enableRemoteAudit {
		...
		if err := apply.Default(conf, conf.AuditLogsProject, opts.Runner); err != nil {
	...
	if conf.Forseti != nil && wantProject(conf.Forseti.Project.ID) {
		...
		if err := apply.Forseti(conf, opts.Runner); err != nil {
	...
	if enableRemoteAudit && conf.AllGeneratedFields.Forseti.ServiceAccount != "" {
		...
		if err := apply.GrantForsetiPermissions(conf.AuditLogsProject.ID, conf.AllGeneratedFields.Forseti.ServiceAccount, opts.Runner); err != nil {
	...
	for _, p := range conf.Projects {
		...
		if err := apply.Default(conf, p, opts.Runner); err != nil {
